...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className="flex h-[100dvh] w-full overflow-hidden bg-surface-muted text-ink pt-[env(safe-area-inset-top)]">
      <Sidebar
      ...
      <div className="workspace-canvas flex h-full min-w-0 flex-1 flex-col">
        <WorkspaceHeader onOpenSidebar={() => setIsSidebarOpen(true)} />

        <main
          className="flex w-full min-h-0 flex-1 flex-col overflow-hidden pb-[max(0.5rem,env(safe-area-inset-bottom),var(--keyboard-inset,0px))]"
        >
          {children}
        </main>
      </div>
